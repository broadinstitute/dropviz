...
gene.descriptions <- right_join(gene.descriptions, unique(all.genes[,'gene',drop=FALSE]), by=c(Associated.Gene.Name='gene')) %>% # limit to genes used in analysis
  mutate(Description=ifelse(is.na(Description),Associated.Gene.Name,Description)) # Some symbols are missing from description (e.g. Drd1) #17
