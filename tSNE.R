...
tsne.label <- function(is.global=TRUE, show.subclusters=FALSE, show.cells=TRUE, show.bags=FALSE, diff.genes=tibble(), comps=tibble(), return.closure=FALSE) {
  function(progress=NULL) {
    ...
    p.func <- function() {
      ...
      tsne.gg <- ggplot() + scale_fill_discrete(guide="none", na.value='lightgray') + scale_size(guide="none", range=c(0.1,opt.expr.size)) + theme_few() + scale_colour_few() + theme(strip.text=element_text(size=20))
